...
const DriversPage: React.FC = () => {
  const [params, setParams] = useState({ page: 1, role: 'driver', status: '', search: '', limit: 10 });
  ...
  return (
    <div className="space-y-8 p-4 sm:p-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-xl sm:text-2xl lg:text-3xl font-black text-primary tracking-tight">Drivers</h2>
          <p className="text-on-surface-variant font-medium">Manage registered drivers on the platform.</p>
        </div>
      ...
      <div className={`bg-white rounded-xl shadow-[0_4px_12px_rgba(26,43,60,0.05)] border border-slate-50 overflow-x-auto ${loading ? 'opacity-50 pointer-events-none' : ''}`}>
          ...
          <table className="w-full min-w-[640px] text-left">
            <thead className="bg-slate-50">
              <tr>
                ...
                <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Vehicle Type</th>
                <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">License</th>
                <th className="px-6 py-4 text-[10px] font-black text-slate-500 uppercase tracking-widest">Status</th>
                <th className="px-6 py-4 text-right text-[10px] font-black text-slate-500 uppercase tracking-widest">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50">
              {(data?.items as DriverUser[])?.map((user) => (
                <tr key={user.userId} className="hover:bg-slate-50/50 transition-colors">
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-blue-100 overflow-hidden border border-blue-200 flex items-center justify-center font-bold text-blue-700">
                        {user.name.charAt(0)}
                      </div>
                      <div>
                  ...
                  <td className="px-6 py-4 text-sm text-[#44474C] font-medium">
                    {user.driverProfile?.vehicleType || 'N/A'}
                  </td>
                  <td className="px-6 py-4">
                    {user.driverProfile?.licenseVerified ? (
        ...
        <div className="p-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
          <p className="text-xs text-slate-500 font-bold">Showing {data?.items.length || 0} of {data?.total || 0} drivers</p>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center">
              <h3 className="text-xl font-black text-primary">Driver Profile</h3>
            ...
            <div className="p-4 sm:p-6 lg:p-8">
              <div className="flex items-center gap-4 sm:gap-6 mb-6 sm:mb-8">
                <div className="w-16 h-16 sm:w-24 sm:h-24 rounded-2xl bg-blue-100 flex items-center justify-center text-2xl sm:text-3xl font-black text-blue-700 border-2 border-blue-200">
                  {selectedUser.name.charAt(0)}
                </div>
                <div>
                  <h4 className="text-2xl font-black text-primary">{selectedUser.name}</h4>
                  <p className="text-slate-500 font-bold uppercase tracking-wider text-xs">Driver</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-bold text-primary">{selectedUser.driverProfile?.licenseNumber || 'N/A'}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">License Status</p>
